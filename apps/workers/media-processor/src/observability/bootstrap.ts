 ...
 * worker-media-processor (ADR-0002).
 *
 ...
 * instrumented module (`@nestjs/core`, `http`, ...) is required, so
 ...
 *
 * The processor's domain metrics live in `MediaProcessorMetrics` (service-local);
 * the worker wires `ObservabilityModule.forRoot({ httpMetrics: false })` because
 * its only HTTP surface is the health probes + the scrape route. See PDD §20.5 +
...
createObservabilityBootstrap('worker-media-processor');
