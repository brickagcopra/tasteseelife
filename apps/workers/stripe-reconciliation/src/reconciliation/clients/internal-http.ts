 ...
 * violates the published contract. The orchestrator catches these so a
 * failed nightly run logs + re-arms rather than crashing the worker.
 */
 ...
 * contract-validated body. Uses Node's built-in `fetch` (Node 22+) with an
 * `AbortController`-backed timeout so a stalled upstream never hangs the
 * run. The body is parsed against the supplied Zod schema so a malformed
 * payload surfaces as a typed `InternalHttpError` rather than propagating a
 * half-shaped object. Response bodies are NEVER echoed into the thrown error
 * (they can carry sensitive figures) — only a truncated trace log.
 */
