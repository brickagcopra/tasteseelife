...
async function bootstrap(): Promise<void> {
  ...
  const logger = createLogger({
    service: 'worker-stripe-reconciliation',
  ...
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'worker-stripe-reconciliation shutdown signal received');
    try {
      await app.close();
    } finally {
  ...
  logger.info(
    {
      ...
      enabled: env.STRIPE_RECONCILIATION_ENABLED,
      runHourUtc: env.STRIPE_RECONCILIATION_RUN_HOUR_UTC,
      tickMs: env.STRIPE_RECONCILIATION_SCHEDULER_TICK_MS,
      accountingServiceBaseUrl: env.ACCOUNTING_SERVICE_BASE_URL,
    },
    'worker-stripe-reconciliation listening',
  );
}
