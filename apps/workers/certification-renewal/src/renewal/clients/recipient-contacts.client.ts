 ...
 * (`POST /api/v1/internal/identity/recipient-contacts`, REUSED from
 * TS-235). Resolves a batch of userIds to a `userId → contact` map so the
 * orchestrator can address + filter recipients. Unknown ids are simply
 * absent from the map.
 */
@Injectable()
