 ...
 * (`POST /api/v1/internal/notification/dispatch`, REUSED from TS-073). The
 * dispatch endpoint renders the template + applies the preference /
 * quiet-hours gate server-side and returns a `DispatchResponse` (status
 * `sent` / `suppressed_*` / `failed`) with HTTP 200. A non-2xx (e.g. 404
 * template missing, 422 variable mismatch, 401 bad secret) is a transport
 * error the client throws — the orchestrator counts it and moves on.
 *
 * Replay safety: the request carries a deterministic `idempotencyKey`
 * (`cert-renewal:{certificationId}:{milestoneDays}`), so re-scanning the
 * same certification on the next day's run collapses against the original
 * dispatch row (`replayed: true`) rather than double-sending a milestone.
 */
@Injectable()
