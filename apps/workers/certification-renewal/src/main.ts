...
async function bootstrap(): Promise<void> {
  ...
  const logger = createLogger({
    service: 'worker-certification-renewal',
  ...
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'worker-certification-renewal shutdown signal received');
    try {
      await app.close();
    } finally {
  ...
  logger.info(
    {
      ...
      enabled: env.CERTIFICATION_RENEWAL_ENABLED,
      runHourUtc: env.CERTIFICATION_RENEWAL_RUN_HOUR_UTC,
      horizonDays: env.CERTIFICATION_RENEWAL_HORIZON_DAYS,
      academyServiceBaseUrl: env.ACADEMY_SERVICE_BASE_URL,
      identityServiceBaseUrl: env.IDENTITY_SERVICE_BASE_URL,
      notificationServiceBaseUrl: env.NOTIFICATION_SERVICE_BASE_URL,
    },
    'worker-certification-renewal listening',
  );
}
