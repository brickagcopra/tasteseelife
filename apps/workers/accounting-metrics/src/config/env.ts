...
/**
 * Environment-variable schema for worker-accounting-metrics (TS-260).
 *
 ...
 *
 * The worker is a thin scheduled trigger with NO datastore of its own.
 * It makes one shared-secret-pinned internal call per day to
 * service-accounting's `POST /api/v1/internal/accounting/saas-metrics/compute`,
 * which owns the ledger read + the money math. The worker therefore needs
 * one base URL + one shared secret + the nightly-cadence knobs.
 *
 * The shared secret matches service-accounting's
 * `INTERNAL_POST_JOURNAL_API_KEY` (the single trust principal for every
 * `/api/v1/internal/*` endpoint on the accounting service) — rotation is
 * a two-side deploy.
 */
const EnvSchema = z
  .object({
    ...
    /**
     * Default port `3053` — next free in the worker block (outbox-relay
     * 3050, identity-janitor 3051, wellness-summary 3052). Used only by
     * the `/healthz` + `/readyz` probes; the work runs on a polling timer.
     */
    PORT: z.coerce.number().int().positive().default(3053),
     ...
     */
    ACCOUNTING_SAAS_METRICS_INTERNAL_API_KEY: z
      .string()
      .min(32, 'ACCOUNTING_SAAS_METRICS_INTERNAL_API_KEY must be at least 32 characters'),
    /** Header carrying the shared secret. Matches the accounting controller. */
    ACCOUNTING_SAAS_METRICS_INTERNAL_HEADER_NAME: z
      .string()
      .min(1)
     ...
     * the nightly compute without redeploying. String env → boolean.
     */
    ACCOUNTING_METRICS_ENABLED: z
      .enum(['true', 'false'])
      .default('true')
      .transform((v) => v === 'true'),

    /**
     * Hour-of-day (UTC) the nightly compute is allowed to fire. Default
     * 02:00 UTC — after the daily revenue-recognition sweep has settled
     * the deferred-revenue balances the metrics read from.
     */
    ACCOUNTING_METRICS_RUN_HOUR_UTC: z.coerce.number().int().min(0).max(23).default(2),
     ...
     * configured hour. The in-process last-run-day guard + the
     * idempotent (date-keyed) compute make a missed/duplicate tick
     * harmless.
     */
    ACCOUNTING_METRICS_SCHEDULER_TICK_MS: z.coerce
      .number()
      .int()
      .positive()
      .max(86_400_000)
...
export class EnvValidationError extends Error {
  constructor(public readonly issues: z.ZodIssue[]) {
    super(`worker-accounting-metrics env validation failed: ${EnvValidationError.format(issues)}`);
