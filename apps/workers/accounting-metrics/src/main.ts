...
async function bootstrap(): Promise<void> {
  ...
  const logger = createLogger({
    service: 'worker-accounting-metrics',
  ...
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'worker-accounting-metrics shutdown signal received');
    try {
      await app.close();
    } finally {
  ...
  logger.info(
    {
      ...
      enabled: env.ACCOUNTING_METRICS_ENABLED,
      runHourUtc: env.ACCOUNTING_METRICS_RUN_HOUR_UTC,
      tickMs: env.ACCOUNTING_METRICS_SCHEDULER_TICK_MS,
      accountingServiceBaseUrl: env.ACCOUNTING_SERVICE_BASE_URL,
    },
    'worker-accounting-metrics listening',
  );
}
