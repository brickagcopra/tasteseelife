...
// (TS-142-followup-4; CLAUDE.md §10; PDD §20.5).
...
async function bootstrap(): Promise<void> {
  ...
  // metrics reaches the collector (TS-142-followup-4). `app.close()`
  // drains the in-flight poll cycle via the scheduler's shutdown hook first.
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'worker-outbox-relay shutdown signal received');
    try {
      await app.close();
    } finally {
