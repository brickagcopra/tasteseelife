 ...
 * discipline. service-media's row-level access model is "the owner sees
 * only their own assets; an admin with `media:read` sees any asset" —
 * the cross-service ownership check ("is this user a member of household
 * X?") is the per-domain row-level filter deferred to TS-110-followup-9.
 * The tenant-scope gate is the cross-cutting belt-and-braces wrapper
 * around every Prisma operation, not a replacement for that filter.
 */
@Injectable()
 ...
 * callbacks. Re-exported for explicit `(tx: PrismaTransactionClient)`
 * annotations at the call sites that need transactional consistency
 * (e.g. the scan-event ingest writes both the event row and the asset
 ...
 *     (`mediaAsset`, `mediaAssetEvent`) and `$transaction` /
 *     `$queryRaw` / `$executeRaw` route here so the extension's
 *     `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.mediaAsset.findUnique()` still
