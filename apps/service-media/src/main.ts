...
import { isS3StubMode, loadEnv } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'service-media shutdown signal received');
    try {
      await app.close();
    } finally {
