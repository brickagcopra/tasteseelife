 ...
 * (`new PrismaService(...)`) — used by integration-test setup — bypasses
 * the wrapper deliberately; those callers own their own scope discipline.
 *
 * service-messaging's row-level access model is "a user sees only threads
 * they're a participant of"; the explicit `thread_participants` membership
 * row remains the trust gate that the realtime layer reads. The tenant-
 * scope SDK is the cross-cutting belt-and-braces wrapper around every
 * Prisma operation — not a replacement for the per-domain membership
 * check.
 */
@Injectable()
 ...
 * `(tx: PrismaTransactionClient)` annotations — once the thread + first-
 * message create flow lands (TS-070-followup), the per-thread metadata
 * insert + the per-participant insert run inside one `$transaction` so
 * the membership row is never visible without its parent thread row.
 */
 ...
 *
 *   - everything else → extended client. Model accessors (`thread`,
 *     `threadParticipant`, ...) and `$transaction` / `$queryRaw` /
 *     `$executeRaw` route here so the extension's `$allOperations`
 *     hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.threadParticipant.findUnique()`
