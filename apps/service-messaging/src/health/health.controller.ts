...
interface LivenessResponse {
  readonly status: 'ok';
  readonly service: 'service-messaging';
 ...
 *              is healthy AND Redis (the Socket.IO adapter backplane
 *              from TS-071, an explicit durable dependency per
 *              CLAUDE.md §4.3) is reachable. Used by Service load
 *              balancers to remove unready pods from rotation. A 503
 *              here pulls the pod out of the LB pool but does NOT
 *              restart it.
 *
 * The split matches PDD §20.2 (K8s topology) and the `/healthz`
 * HEALTHCHECK pre-wired into the canonical Dockerfile (TS-010).
 *
 * TS-070-followup-1 will add a `cassandra: 'ok'` field to the
 * readiness check once the Cassandra cold-store driver is wired —
