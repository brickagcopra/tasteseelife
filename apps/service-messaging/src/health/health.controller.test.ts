import { ServiceUnavailableException } from '@nestjs/common';
import { describe, expect, it, vi } from 'vitest';

import type { Env } from '../config/env';
import type { PrismaService } from '../prisma/prisma.service';
import { HealthController } from './health.controller';
import type { RedisHealthService } from './redis-health.service';

function buildEnv(overrides: Partial<Env> = {}): Env {
  return {
    NODE_ENV: 'test',
    PORT: 3017,
    LOG_LEVEL: 'info',
    DATABASE_URL: 'postgresql://test:test@localhost:5432/test',
    SERVICE_VERSION: 'dev',
    OTEL_TRACES_ENABLED: false,
    OTEL_METRICS_ENABLED: false,
    REDIS_URL: 'redis://localhost:6379',
    REDIS_KEY_NAMESPACE_PREFIX: 'test:service-messaging:socket:',
    WS_PATH: '/socket.io',
    WS_CORS_ORIGINS: '',
    JWT_ACCESS_SECRET: 'a'.repeat(32),
    INTERNAL_TRUST_SIGNING_SECRET: 't'.repeat(32),
    INTERNAL_TRUST_MAX_AGE_SECONDS: 60,
    JWT_ISSUER: 'taste-and-see/service-identity',
    JWT_AUDIENCE: 'taste-and-see/api',
    IDEMPOTENCY_TTL_SECONDS: 86_400,
    IDEMPOTENCY_IN_FLIGHT_TTL_SECONDS: 60,
    ...overrides,
  };
}

function buildRedisHealth(ok: boolean): RedisHealthService {
  return { ping: vi.fn().mockResolvedValue(ok) } as unknown as RedisHealthService;
}

describe('HealthController', () => {
  it('liveness returns a static ok payload', () => {
    const prisma = { ping: vi.fn() } as unknown as PrismaService;
    const redisHealth = buildRedisHealth(true);
    const controller = new HealthController(
      prisma,
      redisHealth,
      buildEnv({ SERVICE_VERSION: 'v1.2.3' }),
    );

    const response = controller.liveness();

    expect(response.status).toBe('ok');
    expect(response.service).toBe('service-messaging');
    expect(response.version).toBe('v1.2.3');
    expect(typeof response.uptimeSeconds).toBe('number');
    expect(response.uptimeSeconds).toBeGreaterThanOrEqual(0);
    // Critically: liveness must NOT consult Postgres OR Redis (a
    // transient blip in either should not restart the pod).
    expect(prisma.ping).not.toHaveBeenCalled();
    expect(redisHealth.ping).not.toHaveBeenCalled();
  });

  it('readiness pings Postgres + Redis and returns ok when both succeed', async () => {
    const prisma = {
      ping: vi.fn().mockResolvedValue(undefined),
    } as unknown as PrismaService;
    const redisHealth = buildRedisHealth(true);
    const controller = new HealthController(prisma, redisHealth, buildEnv());

    const response = await controller.readiness();

    expect(response.status).toBe('ok');
    expect(response.checks.postgres).toBe('ok');
    expect(response.checks.redis).toBe('ok');
    expect(prisma.ping).toHaveBeenCalledTimes(1);
    expect(redisHealth.ping).toHaveBeenCalledTimes(1);
  });

  it('readiness throws ServiceUnavailable when the Postgres ping fails', async () => {
    const prisma = {
      ping: vi.fn().mockRejectedValue(new Error('connection refused')),
    } as unknown as PrismaService;
    const controller = new HealthController(prisma, buildRedisHealth(true), buildEnv());

    await expect(controller.readiness()).rejects.toBeInstanceOf(ServiceUnavailableException);
  });

  it('readiness throws ServiceUnavailable when the Redis ping fails (TS-071 backplane is a hard dep)', async () => {
    const prisma = {
      ping: vi.fn().mockResolvedValue(undefined),
    } as unknown as PrismaService;
    const controller = new HealthController(prisma, buildRedisHealth(false), buildEnv());

    try {
      await controller.readiness();
      throw new Error('readiness unexpectedly succeeded');
    } catch (err) {
      expect(err).toBeInstanceOf(ServiceUnavailableException);
      const body = (err as ServiceUnavailableException).getResponse() as Record<string, unknown>;
      expect(body['detail']).toBe('redis readiness check failed');
      expect(body['title']).toBe('Service Unavailable');
      expect(body['status']).toBe(503);
      expect(body['instance']).toBe('/readyz');
    }
  });

  it('readiness 503 body is RFC 7807-shaped with a postgres-only detail (no driver leakage)', async () => {
    const prisma = {
      ping: vi
        .fn()
        .mockRejectedValue(new Error('FATAL: password authentication failed for user "postgres"')),
    } as unknown as PrismaService;
    const controller = new HealthController(prisma, buildRedisHealth(true), buildEnv());

    try {
      await controller.readiness();
      throw new Error('readiness unexpectedly succeeded');
    } catch (err) {
      expect(err).toBeInstanceOf(ServiceUnavailableException);
      const body = (err as ServiceUnavailableException).getResponse() as Record<string, unknown>;
      expect(body['detail']).toBe('postgres readiness check failed');
      expect(body['title']).toBe('Service Unavailable');
      expect(body['status']).toBe(503);
      expect(body['instance']).toBe('/readyz');
    }
  });
});
