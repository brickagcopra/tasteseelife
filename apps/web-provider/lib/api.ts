...
/**
 * Server-side HTTP client for talking to api-gateway (TS-122).
 *
 * Mirrors `apps/web-family/lib/api.ts`. The browser NEVER makes direct
 * calls to the gateway — every request originates from a Next.js server
 * action or a server component, which (a) reads the access token from
 * the portal's own HttpOnly cookie, (b) forwards it as
 * `Authorization: Bearer ...` to the gateway, and (c) returns a typed
 * response shape to its caller.
 *
