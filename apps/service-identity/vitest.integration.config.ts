...
/**
 * service-identity integration-test configuration (TS-009e).
 *
 * Distinct from the unit-test config (`vitest.config.ts`) so the
 * fast unit suite stays pure-Node + millisecond-fast and the
 * slow infra-touching suite is opt-in via the dedicated
 * `pnpm test:integration` script.
 *
 * Integration tests live under `test/integration/**`. Their lifecycle
 * shape changed under TS-009e-followup-2:
 *
 ...
 *     to carve out a per-file database (so two files don't share
 *     rows) and drops it in `afterAll`.
 *   - Net effect vs the pre-TS-009e-followup-2 per-file pattern:
 *     ~4× fewer container starts (5 files share one Postgres +
 *     one Redis), unchanged per-file migration cost.
 *
