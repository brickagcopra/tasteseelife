 ...
 * load.
 *
 * Tenant-scoping (CLAUDE.md §3.2 + §17.10; TS-020-followup-2): the
 * production DI graph wires this service through `PrismaModule`'s
...
/**
 * The client shape Prisma passes to interactive `$transaction`
 * callbacks. Re-exported as a type for explicit `(tx: PrismaTransactionClient)`
 * annotations (TS-293's expiry sweep) — mirrors the sibling re-export in
 * service-booking / service-accounting so callback params never fall to
 * implicit `any` under this tsconfig's strictness.
 */
 ...
 *
 *   - everything else → extended client. Model accessors (`user`,
 *     `session`, `kycRecord`, ...) and `$transaction` / `$queryRaw` /
 *     `$executeRaw` route here so the extension's `$allOperations`
 *     hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.user.findMany()` still has the
 * same type and the same behaviour, but the query now flows through
