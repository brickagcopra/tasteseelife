...
/**
 * Unit tests for `wrapWithTenantScope` (TS-020-followup-2).
 *
 ...
 * client, and how unknown properties fall back.
 *
 * The tests build a fake "base" PrismaService (matching the pattern
 * used by `auth.service.test.ts` et al — `as unknown as PrismaService`)
...
interface FakeExtendedClient {
  readonly user: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-identity-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.user as unknown as { findMany: () => string }).findMany()).toBe('EXT_USER');
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.user.findMany(...)`,
    ...
    // callsite in service-identity uses that pattern.)
    const userAccessor = wrapped.user as unknown as { findMany: () => string };
    expect(userAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.user);
  });
