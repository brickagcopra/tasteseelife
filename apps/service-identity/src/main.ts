...
// (TS-020-followup-1; CLAUDE.md §10; PDD §20.5).
...
async function bootstrap(): Promise<void> {
  ...
  // metrics reaches the collector (TS-020-followup-1).
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'service-identity shutdown signal received');
    try {
      await app.close();
    } finally {
