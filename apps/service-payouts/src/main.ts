...
import { isStripeStubMode, loadEnv } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'service-payouts shutdown signal received');
    try {
      await app.close();
    } finally {
