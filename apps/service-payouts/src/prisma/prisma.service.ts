 ...
 * — used by future integration-test harnesses — bypasses the wrapper
 * deliberately; those callers own their own scope discipline.
 */
@Injectable()
 ...
 * (e.g. stripe-event ingest writes both the event row and the account
 ...
 *     `stripeAccountEvent`, `payoutDisbursement`) and `$transaction` /
 *     `$queryRaw` / `$executeRaw` / `$executeRawUnsafe` route here so
 *     the extension's `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.providerPayoutAccount.findMany()`
