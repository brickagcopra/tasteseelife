 ...
 *
 * `ping()` exposes a single-row read used by the readiness probe —
 * kept cheap enough to run on every `/readyz` poll without distorting
 * Postgres load.
 *
 * Tenant-scoping (CLAUDE.md §3.2 + §17.10;
 * TS-020-followup-2b-platform-rollout): the production DI graph wires
 * this service through `PrismaModule`'s factory provider
 ...
 * the tenant-scope gate. Direct instantiation (`new PrismaService(...)`)
 * — used by `seed-chart-of-accounts.ts` and any future integration-test
 * harness — bypasses the wrapper deliberately; those callers own their
 * own scope discipline (the seed touches the catalog with the
 * unwrapped client; a test harness is for setup/inspection and doesn't
 * go through the production code path).
 */
@Injectable()
 ...
 *     `periodLifecycleEvent`, `outboxConsumerDedup`) and `$transaction`
 *     / `$queryRaw` / `$executeRaw` / `$executeRawUnsafe` route here so
 *     the extension's `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.chartOfAccount.findMany()` still
 * has the same type and the same behaviour, but the query now flows
