 ...
 * with three Nest lifecycle methods that get exercised by integration
 * tests. These unit tests pin the Proxy routing logic: which property
 * reads bind to the base, which bind to the extended client, and how
 * unknown properties fall back. They mirror the canonical shape landed
 * in `service-subscription` / `service-audit` under
 * TS-020-followup-2b-platform-rollout.
 *
 * The tests build a fake "base" PrismaService (matching the pattern
 * used elsewhere in this service — `as unknown as PrismaService`) with
 * a stubbed `$extends` so the production wrapper sees a known
...
interface FakeExtendedClient {
  readonly chartOfAccount: { findUnique: (this: unknown) => string };
  readonly $transaction: () => string;
  readonly $queryRaw: () => string;
  readonly $executeRaw: () => string;
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-accounting-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.chartOfAccount as unknown as { findUnique: () => string }).findUnique()).toBe(
      'EXT_COA',
    );
  });
  ...
  it('routes $executeRaw through the extended client (PgConsumerDedupStore writes flow through the gate)', () => {
    // The `PgConsumerDedupStore` SDK uses `$executeRaw` for its dedup
    // table writes — pin that the wrapper still routes through the
    // extended client, where the gate's `proceed_unscoped_operation`
    // decision applies via `DEFAULT_UNSCOPED_OPERATIONS`.
    const fakeExtended = makeFakeExtended();
    const base = makeFakeBase(fakeExtended);
    const wrapped = wrapWithTenantScope(
      base as unknown as PrismaService,
    ...
    expect((wrapped.$executeRaw as unknown as () => string)()).toBe('EXT_ER');
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    const fakeExtended = makeFakeExtended({
      chartOfAccount: {
        findUnique(this: unknown): string {
          observedThis = this;
          return 'OK';
        },
      },
    ...
    // The natural Prisma callsite is `this.prisma.chartOfAccount.findUnique(...)`,
    // i.e. method-call syntax on the model accessor. JavaScript binds
    // `this` to the model accessor object on that path — the wrapper
    // doesn't need to do anything special. (Destructuring the method
    // off the model accessor would lose `this`, but no production
    // callsite in service-accounting uses that pattern.)
    const coaAccessor = wrapped.chartOfAccount as unknown as {
      findUnique: () => string;
    };
    expect(coaAccessor.findUnique()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.chartOfAccount);
  });
