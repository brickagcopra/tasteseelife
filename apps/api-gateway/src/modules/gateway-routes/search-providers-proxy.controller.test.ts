...
const VALID_RESPONSE = {
  ...
  nextCursor: null,
  liveMode: false,
  searchId: 'srch_gw_fixture',
};

describe('SearchProvidersProxyController.search', () => {
  it('forwards the validated body and returns the response', async () => {
    ...
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);

    const response = await c.search({ query: 'chef' }, REQUEST_WITH_CTX);
    expect(response).toEqual(VALID_RESPONSE);
    expect(stub.lastOptions?.service).toBe('search');
    expect(stub.lastOptions?.path).toBe('/api/v1/search/providers');
  ...
  it('accepts an empty body (Phase-1 default landing page)', async () => {
    const stub = new StubDownstreamClient({
      kind: 'ok',
      status: 200,
      body: VALID_RESPONSE,
    ...
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);
    const response = await c.search({}, REQUEST_WITH_CTX);
    expect(response).toEqual(VALID_RESPONSE);
  });
  ...
  it('rejects an unknown query field with 400 (strict)', async () => {
    ...
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);
    await expect(c.search({ smuggled: 1 }, REQUEST_WITH_CTX)).rejects.toBeInstanceOf(HttpException);
  });
  ...
  it('throws Unauthorized when no requestContext', async () => {
    ...
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);
    await expect(
      c.search({}, { headers: {} } as unknown as RequestWithContext),
  ...
  it('re-throws 4xx client errors with the downstream status', async () => {
    ...
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);
    await expect(c.search({}, REQUEST_WITH_CTX)).rejects.toMatchObject({ status: 422 });
  });
  ...
  it('maps 5xx server_error to 502', async () => {
    ...
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);
    await expect(c.search({}, REQUEST_WITH_CTX)).rejects.toBeInstanceOf(BadGatewayException);
  });

  it('maps timeout to 504', async () => {
    const stub = new StubDownstreamClient({ kind: 'timeout' });
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);
    await expect(c.search({}, REQUEST_WITH_CTX)).rejects.toBeInstanceOf(GatewayTimeoutException);
  });
  ...
  it('maps not_configured to 503', async () => {
    ...
    const c = new SearchProvidersProxyController(stub as unknown as DownstreamHttpClient);
    await expect(c.search({}, REQUEST_WITH_CTX)).rejects.toBeInstanceOf(
      ServiceUnavailableException,
    );
  });
