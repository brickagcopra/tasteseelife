...
/**
 * Family concierge-assignment BFF proxy (TS-222; PRD §5.1 Tier 3).
 *
 *   `GET /api/v1/concierge/assignments/me`
 *     The active dedicated-concierge assignment for the actor's household
 *     — powers the family portal "Your concierge" card. Authenticated
 *     under the default rate-limit policy. service-concierge resolves the
 *     household from the token's `tenantScope` claim (no household id
 *     crosses the wire), so the gateway is a thin pass-through forwarding
 *     the verified actor identity.
 *
 * A non-household-scoped actor (admin / partner) receives the downstream
 * 400 verbatim — the `/me` surface is for household members only.
 */
@Controller('api/v1/concierge/assignments')
@UseGuards(AccessTokenGuard, RateLimitGuard)
export class ConciergeAssignmentsProxyController {
  ...
  async getMine(@Req() request: RequestWithContext): Promise<ConciergeAssignmentSnapshotResponse> {
    ...
    const result: DownstreamResult = await this.downstream.call({
      service: 'concierge',
      path: '/api/v1/concierge/assignments/me',
      method: 'GET',
      actor: ctx,
      traceId,
    });
