...
import {
  ...
  seedAcademyCertificationRenewalTemplate,
} from './academy-certification-renewal-template';

describe('buildAcademyCertificationRenewalTemplateSeed', () => {
  const seed = buildAcademyCertificationRenewalTemplateSeed();

  it('targets the shared template code, en_US, email kind', () => {
    expect(seed.code).toBe(ACADEMY_CERTIFICATION_RENEWAL_TEMPLATE_CODE);
 ...
 * Mirrors the wellness-summary seed test.
 */
...
describe('seedAcademyCertificationRenewalTemplate', () => {
  ...
  it('creates the template + activates version 1 when nothing exists', async () => {
    ...
    const report = await seedAcademyCertificationRenewalTemplate(prisma as any, mjml);
