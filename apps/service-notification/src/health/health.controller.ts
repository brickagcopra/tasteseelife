...
interface LivenessResponse {
  readonly status: 'ok';
  readonly service: 'service-notification';
 ...
 *              unready pods from rotation.
 *
 * The split matches PDD §20.2 (K8s topology) and the `/healthz`
 * HEALTHCHECK pre-wired into the canonical Dockerfile (TS-010).
 *
 * TS-073 will add per-channel SDK reachability checks (Postmark API
 * ping, Twilio account ping, FCM project ping) to the readiness check
