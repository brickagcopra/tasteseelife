...
/**
 * service-booking integration-test configuration (TS-061-followup-4).
 *
 * Mirrors the canonical shape established by service-identity
 * (`apps/service-identity/vitest.integration.config.ts`) and
 * service-household (`apps/service-household/vitest.integration.config.ts`).
 * Distinct from the unit-test config (`vitest.config.ts`) so the fast
 * unit suite stays pure-Node + millisecond-fast and the slow infra-
 * touching suite is opt-in via the dedicated `pnpm test:integration`
 * script.
 *
 ...
 *   - Net effect for service-booking today: one file (recurrence) so
 *     the shared-stack savings are nominal — but adopting the shared
 *     pattern keeps the shape consistent with the rest of the
 *     workspace and gets the wiring right before TS-062-followup-9 /
 *     TS-063-followup-5 land their own files alongside.
 *
