 ...
 * (`new PrismaService(...)`) — used by future seed scripts and integration-
 * test harnesses — bypasses the wrapper deliberately; those callers own
 ...
 * annotations — TS-060-followup-1's `BookingsService` runs writes
 * inside `$transaction` and the explicit annotation makes the no-
 * cross-service-imports invariant (CLAUDE.md §2.3) easy to spot at
 * the call site.
 */
 ...
 *
 *   - everything else → extended client. Model accessors (`booking`,
 *     `bookingRecurrence`, `bookingVisitNote`, `bookingCheckIn`,
 *     `bookingDispute`, `householdTierSnapshot`, `providerTierSnapshot`,
 *     `outboxEvent`) and `$transaction` / `$queryRaw` / `$executeRaw`
 *     route here so the extension's `$allOperations` hook fires on every
 *     operation.
 *
 * Callsites do not change: `this.prisma.booking.findMany()` still has
