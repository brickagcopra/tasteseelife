 ...
 * integration suite (TS-060-followup-3, deferred). These unit tests pin
 * the Proxy routing logic: which property reads bind to the base, which
 ...
 * TS-020-followup-2 and the `service-provider` / `service-subscription`
 * / `service-household` siblings under TS-020-followup-2b-platform-rollout.
 *
...
interface FakeExtendedClient {
  readonly booking: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-booking-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.booking as unknown as { findMany: () => string }).findMany()).toBe(
      'EXT_BOOKING',
    );
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.booking.findMany(...)`,
    ...
    // callsite in service-booking uses that pattern.)
    const bookingAccessor = wrapped.booking as unknown as { findMany: () => string };
    expect(bookingAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.booking);
  });
