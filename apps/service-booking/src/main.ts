...
// (TS-060-followup-4; CLAUDE.md §10; PDD §20.5).
...
async function bootstrap(): Promise<void> {
  ...
  // metrics reaches the collector (TS-060-followup-4).
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'service-booking shutdown signal received');
    try {
      await app.close();
    } finally {
