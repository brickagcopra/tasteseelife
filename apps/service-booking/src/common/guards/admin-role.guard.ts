...
/**
 * Phase-1 admin gate (TS-128 Slice 1) — twin of service-identity's
 * `SuperAdminRoleGuard` (TS-126) and service-subscription's
 * `SuperAdminRoleGuard` (TS-127). Run AFTER `AccessTokenGuard` — relies
 * on `request.requestContext` being populated.
 *
 ...
 *      hard-wired; granular per-permission gating (`booking:read` /
 *      `booking:write` etc.) arrives once per-resource permissions land
 *      on the RBAC catalog (TS-290). At that point this guard becomes a
 *      thin wrapper around `PermissionGuard` from
 *      `@taste-and-see/nest-auth` with the resource strings instead of
 *      the role name.
 *
 * **Why this guard stays local** rather than lifted to
 * `packages/nest-auth`: it binds to booking-service-specific RBAC
 * semantics. The role name `super_admin` is the Slice-1 wire — future
 * slices swap in `booking:adjust` / `booking:read` per-permission
 * gating once TS-290 lands. Until then the per-service guard owns the
 * booking-domain semantics. (TS-052-followup-11a-4 lifted the shared
