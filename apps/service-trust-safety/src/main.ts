...
// (TS-306-followup-1c; CLAUDE.md §10; PDD §20.5).
...
async function bootstrap(): Promise<void> {
  ...
  // metrics reaches the collector (TS-306-followup-1c). `app.close()` runs
  // first so the outbox consumer stops and the BullMQ scheduler drains
  // while the exporters are still alive — a shutdown that drops the final
  // export loses exactly the batch a crashing pod most needs to have sent.
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'service-trust-safety shutdown signal received');
    try {
      await app.close();
    } finally {
  ...
  logger.info(
    ...
    'service-trust-safety listening',
  );
}
