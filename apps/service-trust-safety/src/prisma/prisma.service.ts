 ...
 *
 *   - everything else → extended client. Model accessors (`incident`,
 *     `outboxEvent`, ...) and `$transaction` / `$queryRaw` / `$executeRaw`
 *     route here so the extension's `$allOperations` hook fires on every
 *     operation.
 *
 * Callsites do not change: `this.prisma.incident.findMany()` still has the
 * same type and the same behaviour, but the query now flows through the
 * tenant-scope gate. `this.prisma.$transaction(async (tx) => ...)` yields
 * a `tx` that is the extended client's transaction client, so each model
 * access inside the transaction also fires the gate.
 *
