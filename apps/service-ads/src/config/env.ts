...
/**
 * Environment-variable schema for service-ads.
 *
 ...
 *
 *   - TS-270 — `DATABASE_URL`, `PORT`, `LOG_LEVEL`, `NODE_ENV`,
 *     `SERVICE_VERSION` (skeleton). Only Postgres-side env at this stage
 *     because TS-270 ships the `ad_campaigns` / `ad_creatives` /
 *     `ad_placements` / `ad_targeting_rules` tables + `/healthz` + `/readyz`
 *     only — there is no authenticated HTTP surface yet. This mirrors the
 *     service-concierge (TS-221) / service-messaging (TS-070) "no dead
 *     config in the skeleton" convention.
 *
 *   - TS-271 (campaign admin) brings the access-token-verification cluster
 *     (`JWT_ACCESS_SECRET` / `JWT_ISSUER` / `JWT_AUDIENCE`) for the
 *     `AccessTokenGuard` + `PermissionGuard` (`ads:write`) plus the
 *     idempotency cache (`REDIS_URL` + TTLs) for the first write endpoint —
 *     mirroring the service-concierge / service-booking shape.
 *
 *   - TS-274 (frequency capping) brings the Redis-backed per-user ×
 *     campaign × period counter cluster (CLAUDE.md §3.7); it rides on the
 *     same `REDIS_URL` TS-271 introduces.
 *
 *   - TS-218a (sponsored-listings resolve) brings the internal shared-secret
 *     cluster (`ADS_INTERNAL_HEADER_NAME` / `ADS_INTERNAL_API_KEY`) that pins
 *     the cluster-internal `service-search → service-ads` resolve endpoint.
 *     This is a service-to-service secret (not a user JWT), so it lands ahead
 *     of the TS-271 access-token cluster — the resolve surface has no user
 *     identity. Mirrors service-search's `SEARCH_INDEX_API_KEY`.
 */
const EnvSchema = z
  .object({
    NODE_ENV: z.enum(['development', 'test', 'staging', 'production']).default('development'),
    /**
     * Default port `3024`: identity = 3010, household = 3011,
     ...
     * academy = 3022, analytics = 3023. The ads service gets the
     * next-available port so the local dev runbook stays predictable.
     */
    PORT: z.coerce.number().int().positive().default(3024),
    ...
    /**
     * Header carrying the internal shared secret on the sponsored-listings
     * resolve surface (TS-218a). Defaults to the platform-wide
     * `x-internal-api-key` (matches service-search's index-ingest convention).
     */
    ADS_INTERNAL_HEADER_NAME: z.string().min(1).default('x-internal-api-key'),
    /**
     * Shared secret the `service-search` delivery caller presents on the
     * cluster-internal sponsored-listings resolve endpoint (TS-218a). At least
     * 32 chars so it carries real entropy. Application-layer defence-in-depth
     * on top of the TS-151 NetworkPolicy (CLAUDE.md §3.5). Mirrors
     * service-search's `SEARCH_INDEX_API_KEY`.
     */
    ADS_INTERNAL_API_KEY: z.string().min(32, 'ADS_INTERNAL_API_KEY must be at least 32 characters'),

    // ───────────────────────────────────────────────────────────────────
    // Access-token verification (TS-271a). service-ads consumes JWTs minted
    // by service-identity (TS-022); the AccessTokenGuard verifies the
    // signature, audience, and issuer before any authenticated admin handler
    // (campaign CRUD, gated on `ads:read` / `ads:write`) sees the request.
    // Phase 1 is HS256 with a shared secret. Same contract as
    // service-academy / service-concierge / service-household.
    // ───────────────────────────────────────────────────────────────────
    ...
    // ───────────────────────────────────────────────────────────────────
    // Idempotency cache (TS-271a). Backs the @Idempotent() interceptor
    // exposed by @taste-and-see/nest-idempotency on the campaign-admin write
    // endpoints. CLAUDE.md §3.3 / §17.5. Same wiring shape as
    // service-academy / service-concierge.
    // ───────────────────────────────────────────────────────────────────
    ...
    // ───────────────────────────────────────────────────────────────────
    // Observability (TS-270-followup-1). service-ads adopts the shared
    // `src/observability/bootstrap.ts` + `ObservabilityModule.forRoot` shape
    // (TS-020-followup-1 / TS-042-followup-8) now that it carries authenticated
    // request traffic worth instrumenting (TS-271a campaign admin + TS-272a
    // slot inventory). The auto-instrumented `http` (the internal sponsored-
    // listings resolve call) + `pg` (the campaign / slot-schedule writes) spans
    // stitch onto a single trace, with the named logical spans
    // (`ads.sponsored.resolve`, targeting evaluation) as parents once their
    // domain counters fold in (TS-273-followup-1, TS-218a observability).
    // CLAUDE.md §10; PDD §20.5. Same coercion shape as service-subscription.
    // ───────────────────────────────────────────────────────────────────
...
export class EnvValidationError extends Error {
  constructor(public readonly issues: z.ZodIssue[]) {
    super(`service-ads env validation failed: ${EnvValidationError.format(issues)}`);
