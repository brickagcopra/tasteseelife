...
/**
 * The bootstrap file's side effects run at import time (TS-270-followup-1).
 * We exercise it indirectly: the file calls `initTracing` / `initMetrics`
 * once at module load; subsequent imports inside the same test process are
 * no-ops (Node's module cache). To re-run with different env we use
 * `vi.resetModules()` to clear the cache between cases.
 *
 * The bootstrap reads `process.env` directly (NOT through `loadEnv()`), so
 * swapping `process.env` keys before the import suffices. Mirrors
 * service-subscription's bootstrap.test.ts (TS-042-followup-8).
 */
