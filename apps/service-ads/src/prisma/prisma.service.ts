...
/**
 * The narrowed Prisma client a `$transaction(async (tx) => ...)` callback
 * receives — carries the model accessors + raw helpers but not the
 * connection-lifecycle methods. Exported so services can type the `tx`
 * parameter without reaching into the Prisma namespace at every callsite.
 */
 ...
 * load.
 *
 * Tenant-scoping (CLAUDE.md §3.2 + §17.10; TS-141): the production DI
 ...
 * — used by unit-test setup and any future seed/maintenance script —
 * bypasses the wrapper deliberately; those callers own their own scope
 * discipline (a script wraps in `runWithoutTenantContext`; the unit-test
 * harness is for test setup/inspection and doesn't go through the
 ...
 *
 *   - everything else → extended client. Model accessors (`adCampaign`,
 *     ...) and `$transaction` / `$queryRaw` / `$executeRaw` route here so
 *     the extension's `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.adCampaign.findMany()` still has
 ...
 * yields a `tx` that is the extended client's transaction client, so each
 * model access inside the transaction also fires the gate.
 *
