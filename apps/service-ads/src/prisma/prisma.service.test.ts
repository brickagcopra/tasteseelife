 ...
 * the canonical shape landed in `service-analytics` / `service-concierge`.
 *
 ...
 * test (TS-270 follow-up).
 */

interface FakeExtendedClient {
  readonly adCampaign: { findMany: (this: unknown) => string };
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.adCampaign as unknown as { findMany: () => string }).findMany()).toBe(
      'EXT_CAMPAIGN',
    );
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.adCampaign.findMany(...)`,
    // i.e. method-call syntax on the model accessor. JavaScript binds `this`
    // to the model accessor object on that path — the wrapper doesn't need to
    // do anything special.
    const campaignAccessor = wrapped.adCampaign as unknown as { findMany: () => string };
    expect(campaignAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.adCampaign);
  });
