 ...
 * test (TS-217-prep-3 follow-up).
 */

interface FakeExtendedClient {
  readonly analyticsAggregationRun: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-analytics-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect(
      (wrapped.analyticsAggregationRun as unknown as { findMany: () => string }).findMany(),
    ).toBe('EXT_RUN');
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // `this.prisma.analyticsAggregationRun.findMany(...)`, i.e. method-call
    // syntax on the model accessor. JavaScript binds `this` to the model
    // accessor object on that path — the wrapper doesn't need to do anything
    // special.
    const runAccessor = wrapped.analyticsAggregationRun as unknown as { findMany: () => string };
    expect(runAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.analyticsAggregationRun);
  });
