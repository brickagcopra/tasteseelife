 ...
 *     (`analyticsAggregationRun`, ...) and `$transaction` / `$queryRaw` /
 *     `$executeRaw` route here so the extension's `$allOperations` hook
 *     fires on every operation.
 *
 * Callsites do not change: `this.prisma.analyticsAggregationRun.findMany()`
