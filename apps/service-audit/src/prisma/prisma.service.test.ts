 ...
 * integration suite. These unit tests pin the Proxy routing logic:
 * which property reads bind to the base, which bind to the extended
 * client, and how unknown properties fall back. They mirror the
 * canonical shape landed in `service-subscription` under
 * TS-020-followup-2b-platform-rollout-svc-subscription.
 *
...
interface FakeExtendedClient {
  readonly auditEvent: { findUnique: (this: unknown) => string };
  readonly $transaction: () => string;
  readonly $queryRaw: () => string;
  readonly $executeRawUnsafe: () => string;
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-audit-test',
    environment: 'test',
    ...overrides,
  });
...
const makeFakeExtended = (overrides: Partial<FakeExtendedClient> = {}): FakeExtendedClient => ({
  ...
  $executeRawUnsafe: (): string => 'EXT_ERU',
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.auditEvent as unknown as { findUnique: () => string }).findUnique()).toBe(
      'EXT_AE',
    );
  });
  ...
  it('routes $executeRawUnsafe through the extended client (advisory-lock acquisition flows through the gate)', () => {
    // service-audit's hash-chain write path issues a `SELECT
    // pg_advisory_xact_lock(...)` via `$executeRawUnsafe` — pin that
    // path routes through the extended client so the gate's
    // `proceed_unscoped_operation` decision applies.
    const fakeExtended = makeFakeExtended();
    const base = makeFakeBase(fakeExtended);
    const wrapped = wrapWithTenantScope(
      base as unknown as PrismaService,
    ...
    expect((wrapped.$executeRawUnsafe as unknown as () => string)()).toBe('EXT_ERU');
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.auditEvent.findUnique(...)`,
    ...
    // callsite in service-audit uses that pattern.)
    const aeAccessor = wrapped.auditEvent as unknown as { findUnique: () => string };
    expect(aeAccessor.findUnique()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.auditEvent);
  });
