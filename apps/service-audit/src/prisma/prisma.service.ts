 ...
 * — used by any future integration-test harness or one-shot script —
 * bypasses the wrapper deliberately; those callers own their own scope
 * discipline (a Nest-managed boot path would wrap in
 * `runWithoutTenantContext`; a test harness is for setup/inspection and
 * doesn't go through the production code path).
 */
@Injectable()
 ...
 * annotations — the hash-chain write path runs inside `$transaction`
 * so the per-resource advisory lock + read-prev + insert-new sequence
 * is atomic, and the explicit annotation makes the no-cross-service-
 * imports invariant (CLAUDE.md §2.3) easy to spot at the call site.
 */
 ...
 *
 *   - everything else → extended client. Model accessors (`auditEvent`)
 *     and `$transaction` / `$queryRaw` / `$executeRaw` /
 *     `$executeRawUnsafe` route here so the extension's
 *     `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.auditEvent.findUnique(...)`
 ...
 * client, so each model access inside the transaction also fires the
 * gate.
 *
