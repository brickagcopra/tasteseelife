 ...
 *
 * TS-100 ships skeleton + JWT verification + internal-ingest shared
...
describe('loadEnv', () => {
  ...
  it('accepts a well-formed environment and applies defaults', () => {
    ...
    expect(env.AUDIT_INGEST_HEADER_NAME).toBe('x-internal-api-key');
    expect(env.AUDIT_INGEST_API_KEY).toBe(baseEnv.AUDIT_INGEST_API_KEY);
  ...
  it('throws EnvValidationError when AUDIT_INGEST_API_KEY is missing', () => {
    const { AUDIT_INGEST_API_KEY: _omit, ...rest } = baseEnv;
    expect(() => loadEnv(rest)).toThrow(EnvValidationError);
  });

  it('throws EnvValidationError when AUDIT_INGEST_API_KEY is too short', () => {
    expect(() => loadEnv({ ...baseEnv, AUDIT_INGEST_API_KEY: 'too-short' })).toThrow(
      EnvValidationError,
    );
  });
  ...
  it('accepts a custom AUDIT_INGEST_HEADER_NAME override', () => {
    const env = loadEnv({ ...baseEnv, AUDIT_INGEST_HEADER_NAME: 'x-tns-ingest' });
    expect(env.AUDIT_INGEST_HEADER_NAME).toBe('x-tns-ingest');
  });
});
