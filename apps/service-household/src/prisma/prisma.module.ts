 ...
 * extension (TS-141 / TS-020-followup-2b-platform-rollout) is applied to
 * the DI-resolved instance. The factory:
 *
 ...
 *
 * The wrapper preserves `PrismaService`'s shape, so `this.prisma.household`,
 * `this.prisma.senior`, `this.prisma.memoryRecipe`, `this.prisma.$transaction`,
 * `this.prisma.ping` etc. all continue to work without changes at the
 * callsite. The lifecycle methods (`onModuleInit` / `onModuleDestroy`) run
 * on the base client, not the extended one, so `$connect` / `$disconnect`
 * operate on the underlying connection pool directly.
 *
