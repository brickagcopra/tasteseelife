 ...
 * runtime behaviour — the class itself is a thin `PrismaClient`
 * wrapper with three Nest lifecycle methods. These unit tests pin the
 * Proxy routing logic: which property reads bind to the base, which
 * bind to the extended client, and how unknown properties fall back.
 * They mirror the canonical shape landed in `service-identity` under
 * TS-020-followup-2 and `service-subscription` under
 ...
 * extended-client object. This keeps the unit suite Prisma-engine-
 * free; the real `$extends` is exercised end-to-end when the service
 * boots against a real Postgres in a future integration test.
 */

interface FakeExtendedClient {
  readonly senior: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-household-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.senior as unknown as { findMany: () => string }).findMany()).toBe('EXT_SENIOR');
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.senior.findMany(...)`,
    ...
    // callsite in service-household uses that pattern.)
    const seniorAccessor = wrapped.senior as unknown as { findMany: () => string };
    expect(seniorAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.senior);
  });
