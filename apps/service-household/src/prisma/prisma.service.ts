 ...
 * (`new PrismaService(...)`) — used by unit-test setup and any future
 * seed/maintenance script — bypasses the wrapper deliberately; those
 * callers own their own scope discipline (a script wraps in
 * `runWithoutTenantContext`; the unit-test harness is for test
 * setup/inspection and doesn't go through the production code path).
 */
@Injectable()
 ...
 *
 *   - everything else → extended client. Model accessors (`household`,
 *     `senior`, `emergencyContact`, `memoryRecipe`, `seniorPreference`,
 *     ...) and `$transaction` / `$queryRaw` / `$executeRaw` route here
 *     so the extension's `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.senior.findMany()` still has
