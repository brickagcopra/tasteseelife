...
interface LivenessResponse {
  readonly status: 'ok';
  readonly service: 'service-provider';
 ...
 *              services (a transient Postgres blip should not restart
 *              the pod).
 *
 *   /readyz  — readiness. Returns 200 only when the service can serve
 *              real traffic — currently that means the Postgres pool
 *              is healthy. Used by Service load-balancers to remove
 *              unready pods from rotation. A 503 here pulls the pod
 *              out of the LB pool but does NOT restart it.
 *
 * The split matches PDD §20.2 (K8s topology) and the `/healthz`
 * HEALTHCHECK pre-wired into the canonical Dockerfile (TS-010).
 */
@Controller()
export class HealthController {
  /**
   * TS-305d-followup-2b — the readiness probe used to catch every throw
   * from `ping()` and return a 503 whose only trace of the cause was a
   * `cause` field on the response body. A pod that never joins its
   * Service's endpoints is the single most consequential failure this
   * process can have, and it was the one failure it did not log.
   * Anything that makes readiness fail is now logged at `error` with the
   * real message and stack before the 503 is thrown.
   */
  ...
  async readiness(): Promise<ReadinessResponse> {
    try {
      ...
      this.logger.error(
        `readiness check failed: postgres ping threw ${err instanceof Error ? err.name : typeof err}`,
