 ...
 * (`new PrismaService(...)`) — used by the seed script and integration-
 * test harness — bypasses the wrapper deliberately; those callers own
 * their own scope discipline (the seed wraps in `runWithoutTenantContext`;
 * the harness is for test setup/inspection and doesn't go through the
...
/**
 * Type alias for the transaction-scoped Prisma client passed into
 * `prisma.$transaction(async (tx) => {...})`. Sourced directly from
 * Prisma's generated `Prisma.TransactionClient` so the generated type
 * stays the source of truth on minor bumps. Imported via `type` so the
 * value-side of the `Prisma` namespace (which resolves inconsistently
 * under our `verbatimModuleSyntax: false` / `isolatedModules: true`
 * tsconfig — see TS-021-followup-2/3) never gets pulled in.
 *
 * TS-052-followup-1 — first consumer is `ProviderCertificationsService`
 * + `TierPromotionService`'s outbox wiring; the `tx` argument the
 * Prisma `$transaction` callback receives carries both the model
 * delegates AND the `$executeRaw` surface the outbox SDK needs.
 */
 ...
 *
 *   - everything else → extended client. Model accessors (`provider`,
 *     `providerApplication`, `providerBackgroundCheck`, `certification`,
 *     `providerCertification`, `providerTierHistory`,
 *     `providerProfileTag`, `outboxEvent`) and `$transaction` /
 *     `$queryRaw` / `$executeRaw` route here so the extension's
 *     `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.provider.findMany()` still has
 * the same type and the same behaviour, but the query now flows through
 * the tenant-scope gate. `this.prisma.$transaction(async (tx) => ...)`
 * yields a `tx` that is the extended client's transaction client, so
 * each model access inside the transaction also fires the gate.
 *
