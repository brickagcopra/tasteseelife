 ...
 * canonical shape landed in `service-identity` under TS-020-followup-2
 * and the `service-subscription` / `service-household` siblings landed
 * under TS-020-followup-2b-platform-rollout.
 *
...
interface FakeExtendedClient {
  readonly provider: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-provider-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.provider as unknown as { findMany: () => string }).findMany()).toBe('EXT_PROV');
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.provider.findMany(...)`,
    ...
    // callsite in service-provider uses that pattern.)
    const provAccessor = wrapped.provider as unknown as { findMany: () => string };
    expect(provAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.provider);
  });
