 ...
 * wrapper with three Nest lifecycle methods that get exercised by the
 ...
 * client, and how unknown properties fall back. They mirror the canonical
 * shape landed in `service-identity` under TS-020-followup-2.
 *
 ...
 * free; the real `$extends` is exercised in the integration tests.
 */

interface FakeExtendedClient {
  readonly subscription: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-subscription-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.subscription as unknown as { findMany: () => string }).findMany()).toBe(
      'EXT_SUB',
    );
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.subscription.findMany(...)`,
    ...
    // callsite in service-subscription uses that pattern.)
    const subAccessor = wrapped.subscription as unknown as { findMany: () => string };
    expect(subAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.subscription);
  });
