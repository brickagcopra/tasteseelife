 ...
 * test harness — bypasses the wrapper deliberately; those callers
 * own their own scope discipline (the seed wraps in
 * `runWithoutTenantContext`; the harness is for test setup/inspection
 * and doesn't go through the production code path).
 */
@Injectable()
 ...
 *
 *   - everything else → extended client. Model accessors (`plan`,
 *     `subscription`, `coupon`, ...) and `$transaction` / `$queryRaw` /
 *     `$executeRaw` route here so the extension's `$allOperations`
 *     hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.subscription.findMany()` still
