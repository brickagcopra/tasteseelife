 ...
 *
 * The wrapper preserves `PrismaService`'s shape, so `this.prisma.plan`,
 * `this.prisma.subscription`, `this.prisma.$transaction`, `this.prisma.ping`
 * etc. all continue to work without changes at the callsite. The
 * lifecycle methods (`onModuleInit` / `onModuleDestroy`) run on the base
 * client, not the extended one, so `$connect` / `$disconnect` operate on
