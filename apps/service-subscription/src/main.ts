...
// (TS-042-followup-8; CLAUDE.md §10; PDD §20.5).
...
async function bootstrap(): Promise<void> {
  ...
  // metrics reaches the collector (TS-042-followup-8).
  const onShutdown = async (signal: NodeJS.Signals): Promise<void> => {
    logger.info({ signal }, 'service-subscription shutdown signal received');
    try {
      await app.close();
    } finally {
  ...
  logger.info(
    ...
    'service-subscription listening',
  );
}
