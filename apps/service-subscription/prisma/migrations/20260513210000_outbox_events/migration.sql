-- TS-142 — outbox table for service-subscription (PDD §7.3, CLAUDE.md §5.3).
--
-- Forward-only expand migration. Creates the canonical `outbox_events`
-- table in the `subscription` schema. No mutations to existing rows;
-- no destructive DDL. The shape is identical to every other producer
...
-- this table (CLAUDE.md §4.1).
--
-- Reversal plan:
--   DROP INDEX IF EXISTS "subscription"."outbox_events_undispatched_idx";
--   DROP TABLE IF EXISTS "subscription"."outbox_events";
...
CREATE INDEX "outbox_events_event_name_idx"
    ON "subscription"."outbox_events" ("event_name");
...
CREATE INDEX "outbox_events_undispatched_idx"
    ON "subscription"."outbox_events" ("created_at")
