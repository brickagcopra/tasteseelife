...
/**
 * Wraps `PrismaClient` with NestJS lifecycle hooks so the connection pool
 * opens at module init and closes cleanly on shutdown (works in concert
 * with `app.enableShutdownHooks()` in main.ts).
 *
 * `ping()` exposes a single-row read used by the readiness probe — kept
 * cheap enough to run on every `/readyz` poll without distorting Postgres
 * load.
 *
 ...
 * `PrismaService` fires the tenant-scope gate.
 *
 * service-webhook is structurally different from every other Nest service
 * in the platform: the rows it persists (`StripeProcessedEvent`,
 * `CheckrProcessedEvent`) come from THIRD-PARTY signature-verified
 * inbound traffic, not from a tenant-scoped user action. There is no
 * Taste & See tenant axis on either table (no household_id, no
 * provider_id) — they are keyed on the third party's `event_id`. Every
 * controller handler therefore wraps its body in
 * `runWithoutTenantContext(..., '<reason>', ...)` so the gate sees an
 * explicit `exempt` frame rather than failing with
 * `MissingRequestContextError` under the `enforcement: 'enforce'` posture
 * wired in `AppModule`. The wrapping is uniform — Stripe + Checkr
 * receivers both pin the same shape today, and any future inbound
 * integration (Twilio delivery receipts, Apple/Google Pay) gets the same
 * treatment. The reason strings are unique + grep-able so a future
 * audit-log scan can trace every "no-context" Prisma access back to its
 * inbound source.
 *
 * Downstream consumers (TS-142 outbox relay) that take these rows and act
 * on them DO have tenant scope and will enforce it at their own service
 * boundary.
 */
@Injectable()
 ...
 *     (`stripeProcessedEvent`, `checkrProcessedEvent`) and `$transaction`
 *     / `$queryRaw` / `$executeRaw` route here so the extension's
 *     `$allOperations` hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.stripeProcessedEvent.create()`
 * still has the same type and the same behaviour, but the query now flows
 * through the tenant-scope gate. `this.prisma.$transaction(async (tx) =>
 * ...)` yields a `tx` that is the extended client's transaction client,
 * so each model access inside the transaction also fires the gate.
 *
