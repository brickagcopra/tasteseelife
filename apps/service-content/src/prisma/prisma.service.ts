 ...
 *
 *   - everything else → extended client. Model accessors (`page`,
 *     `article`, ...) and `$transaction` / `$queryRaw` / `$executeRaw`
 *     route here so the extension's `$allOperations` hook fires on every
 *     operation.
 *
 * Callsites do not change: `this.prisma.page.findMany()` still has the
