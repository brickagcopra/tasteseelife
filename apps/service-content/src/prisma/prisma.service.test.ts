 ...
 * the canonical shape landed in `service-ads` / `service-analytics` /
 * `service-concierge`.
 *
 ...
 * test (TS-280 follow-up).
 */

interface FakeExtendedClient {
  readonly page: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-content-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.page as unknown as { findMany: () => string }).findMany()).toBe('EXT_PAGE');
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.page.findMany(...)`,
    ...
    // do anything special.
    const pageAccessor = wrapped.page as unknown as { findMany: () => string };
    expect(pageAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.page);
  });
