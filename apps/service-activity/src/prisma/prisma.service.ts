 ...
 * discipline. service-activity's row-level access model is "an actor
 * sees only their own rows" — the user-facing self-view filters by the
 * actor's `userId` server-side, and the admin search adds permission
 * gating (`activity:read`, TS-101-followup-7) once `PermissionGuard`
 * lifts (TS-052-followup-11). The tenant-scope gate is the cross-cutting
 * belt-and-braces wrapper around every Prisma operation, not a
 * replacement for the per-domain row-level filter.
 */
@Injectable()
 ...
 * annotations. service-activity's writes are single-row INSERTs (no
 * multi-statement transaction is required for the Phase-1 record
 * surface) but the type is re-exported for symmetry with the audit-svc
 * shape and for future use when a producer integration grows a
 * co-mutating side effect.
 */
 ...
 *     (`activityEvent`, ...) and `$transaction` / `$queryRaw` /
 *     `$executeRaw` route here so the extension's `$allOperations`
 *     hook fires on every operation.
 *
 * Callsites do not change: `this.prisma.activityEvent.findMany()` still
 ...
 * => ...)` yields a `tx` that is the extended client's transaction
