/**
 * OpenTelemetry tracing + Prometheus metrics bootstrap for service-concierge
 * (TS-306-followup-1d).
 *
 ...
 *
 * The load-bearing auto-instrumented signal here is `http`: the concierge
 * surfaces aggregate across several services (visit prep, emergency paths),
 * so a slow response here is usually somebody else's slow response and the
 * span tree is what says whose.
 *
...
createObservabilityBootstrap('service-concierge');
