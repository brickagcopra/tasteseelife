 ...
 * the canonical shape landed in `service-household` /
...
interface FakeExtendedClient {
  readonly conciergeTicket: { findMany: (this: unknown) => string };
...
const fakeOptions = (
  ...
  validateOptions({
    serviceName: 'service-concierge-test',
    environment: 'test',
    ...overrides,
  });
...
describe('wrapWithTenantScope', () => {
  it('routes model accessors through the extended client', () => {
    ...
    expect((wrapped.conciergeTicket as unknown as { findMany: () => string }).findMany()).toBe(
      'EXT_TICKET',
    );
  });
  ...
  it('preserves the `this` binding of model methods when called via the natural callsite pattern', () => {
    ...
    // The natural Prisma callsite is `this.prisma.conciergeTicket.findMany(...)`,
    ...
    const ticketAccessor = wrapped.conciergeTicket as unknown as { findMany: () => string };
    expect(ticketAccessor.findMany()).toBe('OK');
    expect(observedThis).toBe(fakeExtended.conciergeTicket);
  });
